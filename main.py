...
#----------Initialization---------------
    ...
    #print(tspike_mpc)
    str1 = 'spikedata_n_' + str(N)+'_c_'+str(round(c,2))+'_w_'+str(w)+nametag+'.pkl'
    with open(str1,'wb') as f:
        pickle.dump([spiketimes_mpc, spikeneurons_mpc],f)
    
    str2 = 'vtraces_n_' + str(N)+'_c_'+str(c)+'_w_'+str(w)+nametag+'.pkl'
    with open(str2,'wb') as g:
        pickle.dump([vpoints],g)    
    
    print('MPC = ',SynMeasures.mpc_network(N,spiketimes_mpc,spikeneurons_mpc)[0])
